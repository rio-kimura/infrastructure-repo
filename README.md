...
## 2. インフラ基盤構成 構成ソフトウェア仕様・技術選定理由

インフラ班がコマンド監査を実行し、各サーバーの内部に自動導入を完了させたミドルウェアの確定バージョンと、それらを「なぜ採用したのか」という技術的選定・設計理由です。

共同開発における「環境パリティ（開発・本番環境の完全一致）」と「実務レベルのセキュリティ」を両立させています。

### 2.1 全台共通の基盤レイヤー

* **OS: AlmaLinux release 9.x (linux/amd64)**  
  * **なぜ採用したか？**: 現在、商用サーバーの現場で最も有力な後継OSとして採用されている「RHEL（Red Hat Enterprise Linux）9」の完全互換OSです。実務に即した長期の安定性と信頼性を重視しました。
* **リソース測定器: Node Exporter 1.8.1**  
  * **なぜ採用したか？**: 各サーバーのCPUやメモリ残量を正確に測るメーターです。各OSの裏側で起動させ、Server CのPrometheusから15秒に1回データを引き抜けるように待機させています。
* **ファイアウォール: Firewalld (OS標準) ＆ SSH変更**  
  * **なぜ採用したか？**: 標準の「22番ポート」を狙ったハッカーからの無差別な自動攻撃を防御するため、接続ドアを **2222番** に変更（要塞化）し、Firewalldで強固にアクセス制御しています。

### 2.2 Server A：司令塔 (Control Node)

* **自動化エンジン: Ansible Core 2.14.18 (Python 3.9系)**  
  * **なぜ採用したか？**: AlmaLinux 9が公式サポートする最も安定した長期サポート（LTS）バージョンです。  
  * **プロの設計ポイント**: Server A（司令塔）にのみAnsible本体を入れ、Server B/C（作業場）側には余計な常駐ソフトを一切入れない「エージェントレス」というスマートな仕組みを活かしています。
* **Docker Engineの導入: 『あえて非導入（設計通り）』**  
  * **なぜ採用したか？**: 司令塔サーバーではアプリを動かさないため、Dockerは入れていません。不要なサービスを徹底的に止めることで、メモリを節約し、ハッカーに踏み台にされる隙をなくす「最小特権の原則」を徹底しています。

### 2.3 Server B：アプリケーション実行環境 (App/DB Node)

* **コンテナ実行基盤: Docker Engine / CLI 29.4.3 (build 56be731 / 055a478)**
* **コンテナランタイム: containerd.io 1.7.27**  
  * **なぜ採用したか？**: 本インフラの最重要ポイントです。開発班がPCで使用しているDockerのバージョン・ビルド番号と、サーバー側の環境を1文字も狂わずに完全一致させました。これにより、共同開発で最も恐ろしい「開発者のPCでは動いたのに、サーバーに載せたらDockerの内部仕様の差で動かない」というデプロイ事故を論理的に根絶しています。
* **一括制御プラグイン: Docker Compose Plugin v2.32.4**  
  * **なぜ採用したか？**: 開発班がPCでの検証用に作成する、複数コンテナの定義書（`docker-compose.yml`）をサーバー側でもそのまま解釈して、Ansibleから一元起動できるようにするため導入しています。
* **自動化通訳ライブラリ: Docker SDK for Python 7.1.0 ＆ requests 2.31.0**  
  * **なぜ採用したか？**: 司令塔のAnsibleが、Server Bに対して「コンテナを作って！」と命令したとき、その命令を通訳してDockerに伝えるためのPythonライブラリです。requestsのバージョンを厳格に固定しているのは、外部の勝手な自動アップデートのせいでAnsibleが突然エラーになるのを防ぐ実務的な安全設計です。
* **隔離ネットワーク: app_network (Driver: bridge)**  
  * **なぜ採用したか？**: コンテナ専用の「見えない社内LAN」です。Webサイト（Nginx）は外部に一般公開しますが、データベース（DB）はこの閉じた `app_network` の中に隠蔽することで、「外部インターネットからのDB侵入ポートを100%遮断し、コンテナ同士の間だけで安全に高速通信させる」という境界防御を確立しています。

### 2.4 Server C：監視管制塔 (Monitor Node)

* **設計思想: 「オール・イン・コンテナ（All-in-Container）」構成への最適化**  
  * **なぜ採用したか？**: Prometheus、Grafana、通知ボットのすべてを、OSに直接入れるのではなく、独立した個別のDockerコンテナ（v29.4.3上）としてカプセル化して稼働させます。これによりホストOS側を汚さず、将来の移設やプログラム更新を極めて簡単（ステートレス）にするモダンな設計を採用しました。
* **監視データベース: Prometheusコンテナ (prom/prometheus:v2.51.0)**  
  * **なぜ採用したか？**: 3台のサーバーのメーターから、15秒に1回健康データを自動回収（Pull）して蓄積する、世界定番の超高速な監視専用データベースです。
* **画面モニター: Grafanaコンテナ (grafana/grafana-oss:latest)**  
  * **なぜ採用したか？**: Prometheusが溜めた難解な数字データを、ブラウザから一目で確認できる美しい折れ線グラフのダッシュボードに変換する可視化ツールです。
* **警備ロボット: NOC統合通知ボットコンテナ (noc-integrated-bot:latest)**  
  * **なぜ採用したか？**: インフラ班内製のPythonスクリプトコンテナです。データの異常（サーバーダウン、メモリ90%超過など）を検知した瞬間、一瞬でDiscordへリッチなアラートメッセージをPOST通知します。

#### 💡 コンテナ運用のための堅牢化設計（プロの工夫）

1. **データの永続化マウント（SELinux `:Z` オプション適用）**  
   コンテナは使い捨てのため、普通に動かすと再起動で「過去のグラフデータ」が消えてしまいます。そのため、データ領域だけはホストOS側（`/var/lib/prometheus` など）に保存されるよう保護しています。その際、AlmaLinux 9のセキュリティ機能（SELinux）による「謎の書き込み拒否エラー」を根絶するため、Ansibleでフォルダにコンテキストを自動適用させ、マウント時に `:Z` フラグを付与しています。
2. **暴走ストッパー設定（Resource Quotas）**  
   万が一、内製通知ボットなどのプログラムがバグで無限ループや大暴走を起こしても、Server C全体のOSメモリやCPUを喰い潰さないように、 `memory: 512M` / `cpus: 0.5` という上限（クォータ）を厳格に課しています。これにより、特定のコンテナが暴走しても、監視システムそのものが巻き添えで共倒れになるリスクをシステム側で完全に防止しています。
...
└── roles/                     # 手順書を機能ごとに役割分担して格納するフォルダ
    ...
    │   ├── tasks/
    │   │   └── main.yml       # PrometheusとGrafanaのインストール、Systemd起動登録、Discord通知ボットの常駐化
    │   ├── templates/
...
> ⚠️ **注意**: 対策済みのPlaybookなので、未定義エラーやDockerのタイムアウトを起こすことなく、一気に最後までノンストップで駆け抜けてオールグリーン（`failed=0`）を叩き出します。これですべての工程が完全自動で復元されました！
...
```
> 💡 **アドバイス**: 上記を実行した後、再度 `vagrant destroy -f` または `vagrant reload` を叩けば、PCを再起動せずとも100%確実に安全な状態からリトライできます。

---

## 7. 監視管制画面（Grafana）の初期登録手順

### 🛠 Step 1: Prometheusが生きているか確認

まず、データ元（Prometheus）がちゃんとデータを集めているか確認します。

1. ブラウザで [http://10.149.245.116:9090](http://10.149.245.116:9090) にアクセスします。
...
1. Grafana（[http://10.149.245.116:3000](http://10.149.245.116:3000)）にログイン。
2. 左メニューの **[Connections]** ➔ **[Data sources]** をクリック。
3. **[Add data source]** ボタンを押し、**Prometheus** を選択。
4. Connection (URL) の欄に、Server CのプライベートIPを使って以下を入力します：  
...
2. **[Import via grafana.com]** の欄に、魔法の数字 **`1860`** を入力して **[Load]** を押します。  
   *(※これは「Node Exporter Full」という、CPUやメモリを完璧に表示してくれる超定番ボードです。)*
...
🚀 **これで完成！** インポートしたダッシュボードで、Server A, B, C の負荷がリアルタイムで動いているはずです。
...
* 📊 **監視管制画面（Grafana）**: [http://localhost:3000](http://localhost:3000) （初期ID/PW: `admin` / `admin`）
* 🗄️ **データ蓄積サーバー（Prometheus）**: [http://localhost:9090](http://localhost:9090)
* 🌐 **Webアプリケーション入口（Nginx）**: [http://localhost:8080](http://localhost:8080) （「Welcome to nginx!」の表示）
...
* **どうやって解決したか**: リポジトリ直下に `.gitattributes` というファイルを配備しました。これにより、Windows上でファイルを編集して普通に `git add .` してコミットするだけで、Gitが裏側で自動的に改行マークをLinux標準の `LF` コードへ綺麗に変換・統一してくれます。
